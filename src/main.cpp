...
v8::Local<v8::String> operator"" _v8(char const *str, size_t) {
...
template <template <typename> typename Service> struct ProxiedService<::APIGenerator, Service> : Named {
  ...
  ProxiedService(std::string const &name) : Named(name), obj(Nan::New<v8::Object>()) {}
...
template <int length, typename O, typename F> void SetFunc(O obj, std::string name, F f) {
  ...
  v8::Local<v8::Function> t =
      v8::Function::New(Nan::GetCurrentContext(), +[] V8CALLBACK { list[info.Data()->ToInteger()->Value()](info); }, Nan::New((int32_t)list.size()), length, v8::ConstructorBehavior::kThrow)
          .ToLocalChecked();
...
template <> struct V8IO<std::string> {
  static constexpr auto length = 1;
  template <typename Info> static api::Buffer read(Info const &info) {
    if (info.Length() < length || !info[0]->IsString())
      Nan::ThrowTypeError("wrong argument");
...
template <> struct V8IO<api::CommandRequest> {
  ...
  template <typename Info> static api::Buffer read(Info const &info) {
    if (info.Length() < length || !info[0]->IsString() || !info[1]->IsString())
      Nan::ThrowTypeError("wrong argument");
...
template <> struct V8IO<api::BlacklistOP<true>> {
  ...
  template <typename Info> static api::Buffer read(Info const &info) {
    if (info.Length() < length || !info[0]->IsString() || !info[1]->IsString() || !info[2]->IsString())
      Nan::ThrowTypeError("wrong argument");
    Nan::Utf8String type(info[0]);
    Nan::Utf8String content(info[1]);
...
struct APIGenerator {
  template <typename T> struct proxied_action : api::Named {
    proxied_action(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
      SetFunc<V8IO<T>::length>(obj, name, [=] V8CALLBACK { apid_invoke(NULL, NULL, api::Buffer::buildKeyName(service_name, name), V8IO<T>::read(info)); });
    }
  };
  template <typename T, typename R> struct proxied_method : api::Named {
    proxied_method(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
  ...
  template <typename T> struct proxied_property : api::Named {
    proxied_property(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
  ...
  template <typename T> struct proxied_broadcast : api::Named {
    proxied_broadcast(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
      SetFunc<1>(obj, name, [=, buffer = api::Buffer::buildKeyName(service_name, name)] V8CALLBACK {
        ...
        apid_subscribe([](char const *data, void *privdata) {
          Nan::HandleScope scope;
  ...
  template <typename T> struct proxied_pattern_broadcast : api::Named {
    proxied_pattern_broadcast(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
      SetFunc<2>(obj, name, [=, buffer = api::Buffer::buildKeyName(service_name, name)] V8CALLBACK {
        ...
        apid_subscribe_pattern([](char const *identify, char const *data, void *privdata) {
          Nan::HandleScope scope;
          auto &cb = *(Nan::Callback *)privdata;
  ...
  template <typename T> struct proxied_set : api::Named {
    proxied_set(std::string const &name) : Named(name) {}
    void $(v8::Local<v8::Object> &obj, std::string service_name) {
...
NAN_MODULE_INIT(InitAll) {
  ...
  Nan::SetMethod(target, "init", [] CALLBACK {
    if (info.Length() == 0) {
      info.GetReturnValue().Set(apid_init());
    } else if (info.Length() == 1 && info[0]->IsString()) {
      Nan::Utf8String str(info[0]);
      info.GetReturnValue().Set(apid_init_unix(*str));
    } else if (info.Length() == 2 && info[0]->IsString() && info[1]->IsNumber()) {
      Nan::Utf8String str(info[0]);
      info.GetReturnValue().Set(apid_init_tcp(*str, info[1]->NumberValue()));
    } else {
  ...
  Nan::SetMethod(target, "attach", [] CALLBACK {
    if (info.Length() == 0) {
      info.GetReturnValue().Set(apid_attach(Nan::GetCurrentEventLoop()));
    } else {
